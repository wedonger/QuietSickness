...
public class BarcoFisica : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        rg.AddForceAtPosition(Physics.gravity / QuantidadeFloater, transform.position, ForceMode.Force);
        
        float altura = Lindoia.instance.GetWaveHeight(transform.position.x);
        if (transform.position.y < altura - 5)
        {
            displacementAmout = (transform.position.y - altura) * -1;
            float displacementMultiplayer = Mathf.Clamp01((altura - transform.position.y - transform.position.y) / profundidadeAntes) * displacementAmout;
